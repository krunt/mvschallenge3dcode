...

{
  ...
  if (forg == NULL || fdest == NULL || fapex == NULL) {
     return OUTSIDE;
  }
...
/*  encountered triangle `de', or from a triangle chosen from a       */
...
int delaunaylocate(struct mesh *m, struct behavior *b,
   REAL * searchpoint, int * outindices)
{
   vertex v1, v2, v3;
   enum locateresult result;
   triangle ptr;
   struct otri horiz, trisym;
   REAL v[2];
   ...
   //if (m->recenttri.tri != NULL) {
   //   otricopy(m->recenttri, horiz);
   //} else {
   {
      traversalinit(&m->triangles);
      horiz.tri = triangletraverse(m);
      horiz.orient = 0;
   }

   result = locate(m, b, searchpoint, &horiz);
   if (result == OUTSIDE)  {
      return -1;
   }
   ...
   //outtri[1] = v1[1];
   //outattrs[0] = 1;
   outindices[0] = vertexmark(v1);

   //outtri[2] = v2[0];
   //outtri[3] = v2[1];
   //outattrs[1] = 1;
   outindices[1] = vertexmark(v2);

   //outtri[4] = v3[0];
   //outtri[5] = v3[1];
   //outattrs[2] = 1;
   outindices[2] = vertexmark(v3);

   //vertex2tri(v1)

   horiz.orient = 1;
   sym(horiz, trisym);
   outindices[3] = * (int *) (trisym.tri + 6);
   horiz.orient = 2;
   sym(horiz, trisym);
   outindices[4] = * (int *) (trisym.tri + 6);
   horiz.orient = 0;
   sym(horiz, trisym);
   outindices[5] = * (int *) (trisym.tri + 6);
...
void triangulate(char *triswitches, struct triangulateio *in,
                 struct triangulateio *out, struct triangulateio *vorout,
                    struct mesh ** meshout, struct behavior ** behavout)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (meshout) {
     *meshout = (struct mesh *)malloc(sizeof(struct mesh));
     **meshout = m;
  }

  if (behavout) {
     *behavout = (struct behavior *)malloc(sizeof(struct behavior));
     **behavout = b;
  }
