...
public:
   ...
   bool GetNeighborsForPoint(const cv::Point2d & pnt, std::vector<TinPoint> & neighbors);
   void Interpolate(MyPointCloud & out);
   void DebugExport(MyPointCloud & out);

   void Visualize(int width, int height) const;
