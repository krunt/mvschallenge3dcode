                      ...
                      const MyCorrWindow &wref, double &ncc) const
{
   ...
   std::vector<double> intensityRef, intensityMy;
   for (int i = 1; i < pointsY; ++i) {
      for (int j = 1; j < pointsX; ++j) {
         ...
         intensityRef.push_back(vref);
         intensityMy.push_back(vmy);
      }
   }
   ...
   for (int i = 0; i < intensityRef.size(); ++i) {
      double vref = intensityRef[i];
      double vmy  = intensityMy[i];
                      ...
                      const MyCorrWindow &wref, const MyLine & refLine, double &ncc) const
{
   ...
   std::vector<double> signRef, signMy;
   std::vector<double> intensityRef, intensityMy;
   for (int i = 1; i < pointsY; ++i) {
      for (int j = 1; j < pointsX; ++j) {
         MyPoint2d refp1 = wref.GetInterpPoint((double)j/pointsX, (double)i/pointsY);
         ...
         signRef.push_back(refSign);
         signMy.push_back(mySign);
      }
   }
   ...
   for (int i = 0; i < intensityMy.size(); ++i) {
      int refSign = signRef[i];
      int mySign = signMy[i];
...
//#pragma optimize("",off)
double MyCorrImage::GetIntensityByLonLat(const MyPoint2d &p, double h) const
{
   double pixX, pixY;
   if (!m_camera.ToPixelCoordinates(p.p[0], p.p[1], h, pixX, pixY)) {
      return -1;
   }
   MyPoint2d pnt(pixX, pixY);
   if (!IsInside(pnt)) {
      return -1;
   }
   return GetIntensity(pnt);
}
//#pragma optimize("",on)
