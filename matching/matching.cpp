...
//struct behavior * behavStruct = NULL;
//
//triangulate("necpz", &in, &out, NULL, &meshStruct, &behavStruct);
//
//cv::namedWindow("myWin");
...
//      bool found = false;
//      if (delaunaylocate(meshStruct, behavStruct, queryPoint, outindices) == 0) {
//         std::set<int> neigh, visited;
//         fetch_point_neighbors(outindices[1], outindices[3], out, visited, neigh);
//         fetch_point_neighbors(outindices[1], outindices[4], out, visited, neigh);
//         fetch_point_neighbors(outindices[1], outindices[5], out, visited, neigh);
//
//         for (auto triIndex: neigh) {
//            int p0 = out.trianglelist[triIndex*out.numberofcorners+0];
//            int p1 = out.trianglelist[triIndex*out.numberofcorners+1];
//            int p2 = out.trianglelist[triIndex*out.numberofcorners+2];
//            int points[3] = {p0,p1,p2};
//            for (int i = 0; i < 3; ++i) {
//               REAL * from = &out.pointlist[2*points[i]];
//               REAL * to = &out.pointlist[2*points[(i+1)%3]];
//               bool fromAttr = out.pointmarkerlist[points[i]] > 1;
//               bool toAttr = out.pointmarkerlist[points[(i+1)%3]] > 1;
//
//               cv::Point fromPoint = cv::Point(from[0], from[1]);
//               cv::Point toPoint = cv::Point(to[0], to[1]);
//
...
//static void fetch_point_neighbors(int pointIdx, int triIdx, struct triangulateio & inp, std::set<int> & visited, std::set<int> & neigh)
//{
//   if (triIdx==-1) {
//      return;
//   }
...
//            fetch_point_neighbors(pointIdx, inp.neighborlist[triIdx*3+i], inp, visited, neigh);
//         }
//      }
//   }
//}