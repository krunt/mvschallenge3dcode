...
struct MyFeatureEdge
{
   ...
   {
      cv::Vec2d v = cv::Vec2d(m_p[1].x-m_p[0].x,m_p[1].y-m_p[0].y);
      return sqrt(v.dot(v));
   }
   cv::Point2d Interpolate(double t) const
   {
      return cv::Point2d(m_p[0].x+t*(m_p[1].x-m_p[0].x), m_p[0].y+t*(m_p[1].y-m_p[0].y));
   }
};
...
struct Gc3Candidate
{
   ...
   {
      return abs(atan(deltaSncc / deltaZ)) / (M_PI / 2);
   }
...
struct Gc3Result
{
   ...
   { 
      ...
      if (m_candidates.empty() || m_candidates[m_candidates.size()-2].GetReliability() == 0) {
      //if (m_candidates.empty() || m_candidates.front().GetSncc() == 0) {
         return -1.0;
      }

      //return m_candidates.back().GetSncc() / m_candidates.front().GetSncc();
      //return m_candidates.back().GetReliability() / m_candidates[m_candidates.size()-2].GetReliability();
      return m_candidates.back().GetSncc() / m_candidates[m_candidates.size()-2].GetSncc();
   }
...
public:
   ...
   Gc3Result * & GetPResultDebugFromCache(int y, int x) { return m_fDebugPointCache[y * m_refImage.GetImage().cols + x]; }
...
private:
   ...
   void StubPerformMatchPointRelaxation();
   ...
   void ConstructEdgeTinStructure(const MyPointCloud & cloud);
   void RemoveInconsistentPoints();
   void DebugShowGc3Result(const std::vector<Gc3Result *> & res);
   void DebugShowGc3Edges(int maxEdges=10);
